...
class MasterTestRunner:
    ...
    @classmethod
    def runTests(cls, run_script, custome_runner_module="", custom_runner_class=""):
        ...
        if custome_runner_module and custom_runner_class:
            CustomRunner = cls.dynamic_import(custome_runner_module, custom_runner_class)
            runner = CustomRunner(resultclass=RunSuite)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--runner_module", help="module of the custom test runner", default="")
    parser.add_argument("--runner_class", help="class of the custom test runner", default="")
    args = parser.parse_args()

    if args.env:
        MasterTestRunner.setEnvironmentVairables(args.env)

    environment_details = MasterTestRunner.collectEnvironmentDetails()
    all_results = []

    start_time = time.time()
    for test_script in args.test_scripts:
        results = MasterTestRunner.runTests(test_script, args.runner_module, args.runner_class)
